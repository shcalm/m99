...
ScopedJavaLocalRef<jobject> CreatePeerConnectionFactoryForJava(
  ...
  dependencies.fec_controller_factory = std::move(fec_controller_factory);//songhua2 FEC is lack of package controller
  ...
  if (!(options && options->disable_network_monitor)) {
    dependencies.network_monitor_factory =
        std::make_unique<AndroidNetworkMonitorFactory>();//songhua2 need to check,what is networkmonitor
  }
